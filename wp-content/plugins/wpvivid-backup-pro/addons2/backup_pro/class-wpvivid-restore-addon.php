...
class WPvivid_Restore_addon
{
    ...
    {
        $backup = WPvivid_Backuplist::get_backup_by_id($this->backup_id);
        if($backup===false)
        {
        ...
        if($this->backup_data['need_download'])
        {
            ...
            <div id="wpvivid_need_download_progress" style="display: none">
                <div style="padding-top:1em;border-top:1px solid #cccccc;">
                    <span class="dashicons dashicons-download wpvivid-dashicons-green"></span>
                    <span>Retrieving backups from cloud</span>
                    <span id="wpvivid_restore_cancel_download" class="dashicons dashicons-dismiss wpvivid-dashicons-grey" style="float:right;cursor: pointer;" title="Click here to cancel the download"></span>
                    <p></p>
                    <span class="wpvivid-span-progress">
                    <span id="wpvivid_restore_download_progress" class="wpvivid-span-processed-progress wpvivid-span-processed-percent-progress" style="width: 0%">0% completed</span>
                </span>
                </div>
                <div id="wpvivid_need_download_list" style="padding: 1em 0;border-bottom:1px solid #f1f1f1;">
                </div>
            </div>
            <script>
                    ...
                    wpvivid_post_request_addon(ajax_data, function (data)
                    {
                        try
                        {
                            ...
                            if (jsonarray.result === 'success')
                            {
                                ...
                                jQuery('#wpvivid_need_download_list').html(jsonarray.list);
                                download_array = new Array();
                                var download_num = 0;
                                jQuery.each(jsonarray.files, function (index, value)
                                {
                                    if (value.status === "need_download")
                                    {
                                        download_array[download_num] = new Array('file_name', 'size');
                                        download_array[download_num]['file_name'] = value.file_name;
                                        download_array[download_num]['size'] = value.size;
                                        download_num++;
                                    }
                                });
                                download_index=0;
                                wpvivid_download_restore_file_ex();
                            }
                            else
                            {
                                alert(jsonarray.error);
                        ...
                        jQuery('#wpvivid_need_download_progress').hide();
                        jQuery('#wpvivid_restore_retrieve_to_local').css({'pointer-events': 'auto', 'opacity': '1'});
                        var error_message = wpvivid_output_ajaxerror('Download backup', textStatus, errorThrown);
                        alert(error_message);
                    });
                });

                function wpvivid_download_restore_file_ex()
                {
                    ...
                    if(download_index+1>download_array.length)
                    {
                        location.reload();
                    }
                    else
                    {
                        var ajax_data = {
                                'action': 'wpvivid_download_restore_file_ex',
                                'backup_id': backup_id,
                                'file_name': download_array[download_index]['file_name'],
                                'size': download_array[download_index]['size'],
                            };
                        download_progress_retry=0;
                        wpvivid_monitor_download_restore_task_ex();
                        wpvivid_post_request_addon(ajax_data, function (data)
                        {
                        }, function (XMLHttpRequest, textStatus, errorThrown)
                        {
                ...
                function wpvivid_monitor_download_restore_task_ex()
                {
                    var ajax_data={
                        'action':'wpvivid_get_download_restore_progress_ex',
                        'backup_id': backup_id,
                        'file_name': download_array[download_index]['file_name'],
                        'size': download_array[download_index]['size'],
                    };
                    ...
                    wpvivid_post_request_addon(ajax_data, function(data)
                    {
                        try
                        {
                            ...
                            if(typeof jsonarray ==='object')
                            {
                                ...
                                {
                                    jQuery('#wpvivid_need_download_list').html(jsonarray.list);
                                    jQuery('#wpvivid_restore_download_progress').width(jsonarray.main_progress+'%');
                                    jQuery('#wpvivid_restore_download_progress').html(jsonarray.main_progress+'% completed');
            ...
            <div id="wpvivid_restore_setting" style="display: none">
                ...
                if(isset($this->backup_data['has_db']))
                {
                    ...
                    //if(isset($this->backup_data['is_db_crypt']))
                    {
                        ...
                        ?>
                        <p><input type="text" option="restore_detail_options" name="restore_db_password" value="<?php echo $password?>"> Enter your password to decrypt the database backup. Ignore it if you didn't enable encryption for this backup.</p>
                        <?php
                    }
                }
                ?>
    ...
    public function retrieve_backup_to_local()
    {
        ...
        $ret['result']='success';
        $files=array();
        foreach ($ret['files'] as $file_name=>$file_data)
        {
                ...
                <div style="padding:0 0 0 2em;width:100%;  box-sizing:border-box;">
                    ...
                    if(isset( $this->backup_data['restore_info']['databases']))
                    {
                        ...
                        <div style="border-bottom:1px solid #cccccc;">
                            ...
                            {
                                ?>
                                <div id="wpvivid_restore_tables_info" style="display: none;height:250px;border:1px solid #ccc;padding:0.2em 0.5em;overflow-y:auto;">
                                    <?php
