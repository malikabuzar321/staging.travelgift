...
if ((isset($_POST['hotel_list']) && !empty($_POST['hotel_list'])) || $hotel_id) {
        ...
        if ($check_st) {
            ...
            if (!empty($room_api_xml_response_for_hotel_search->HotelAvailability)) {
                ...
                foreach ($room_api_xml_response as $hotel) {
                    ...
                    $hotel__id = xml_attribute($hotel_info, 'id');
                    $hotel_img_url = GCTCF_URL . 'public/images/hotel-placeholder-img.png';

                    if (file_exists($directory_path . '/hotel_api/' . $hotel__id . '.xml')) {
                        $xml_hotel_data = file_get_contents($directory_path . '/hotel_api/' . $hotel__id . '.xml');
    ...
    if ($tbo_region) {
        ...
        if ($tbo_mode == 'test') {
            $tbo_username = get_option('options_tboh_test_username');
            $tbo_password = get_option('options_tboh_test_password');
            $tbo_url = get_option('options_tboh_test_url');
        } else {
            $tbo_username = get_option('options_tboh_live_username');
            $tbo_password = get_option('options_tboh_live_password');
            $tbo_url = get_option('options_tboh_live_url');
        }
        ...
        $hotel_search_data = send_tbo_request($tbo_url . 'Search', $hotel_search_request_data, $auth);
        $tbo_available_hotels = $tbo_ids_for_detail = $hotel_details = $tboh_hotels_final = array();
        if (isset($hotel_search_data['HotelResult'])) {
        ...
        if ($tbo_ids_for_detail) {
            $hotel_details_request = send_tbo_request($tbo_url . 'HotelDetails', ['Hotelcodes' => implode(', ', $tbo_ids_for_detail), 'Language' => 'EN'], $auth);
...
if ($gctcf_type) {
    $travellanda_Arr = gctcf_filter_by_board($gctcf_type, $travellanda_Arr);
    $stuba_Arr = gctcf_filter_by_board($gctcf_type, $stuba_Arr);
}

if ($gctcf_prices) {
    $travellanda_Arr = gctcf_filter_hotels_by_price($gctcf_prices,$travellanda_Arr);
    $stuba_Arr = gctcf_filter_hotels_by_price($gctcf_prices,$stuba_Arr);
} else if ($gctcf_ratings) {
    $travellanda_Arr = gctcf_filter_hotels_by_rating($gctcf_ratings, $travellanda_Arr);
    $stuba_Arr = gctcf_filter_hotels_by_rating($gctcf_ratings, $stuba_Arr);
}
                                ...
                                <div class="gc-md-12 gc-sm-12 content_section">
                                    ...
                                    <div class="gc-form-group gc-md-2 hotel_list_both_padding hotel-mobile-margin-top">
                                        <div class="input-group">
                                            <input type="text" class="form-control gctcf-datepicker" value="<?php if (!empty($hotel_start_date)) {
                                                                                                                echo $hotel_start_date;
                                                                                                            } ?>" name="hotel_check_in_date" placeholder="Check in" id="datepicker16" required="required" />
                                    ...
                                    <div class="gc-form-group gc-md-2 hotel_list_both_padding">
                                        <div class="dropdown">
                                            <select class="selectpicker" name="hotel_night" data-style="btn-white">
                                                ...
                                                for ($i = 1; $i <= 15; $i++) {
                                                ?>
                                                    <option <?php if (!empty($hotel_night)) {
                                                                if ($hotel_night == $i) echo 'selected';
                                                            } ?> value="<?php echo $i ?>"><?php echo $i ?> Night</option>
                                                <?php
                                                }
                                                ?>
                                            </select>
...
if (!empty($merged_results)) : ?>
    <section class="section clearfix">
        <div class="total_results" stuba="<?php echo count($_SESSION['stuba_all']); ?>" travellanda="<?php echo count($_SESSION['travelnda_all']); ?>"><?php if (!$hotel_id) : ?> Results: <?php echo count($_SESSION['gctcf_all']);
                                                                                                                                                                                        endif; ?></div>
                ...
                <?php foreach ($merged_results as $merged_result) :  ?>
                    <div class="gc-row hotel__search_result_view">
                        <div class="hotel-image">
                            <!-- Display Hotel Image -->
                            <img src="<?php echo $merged_result['hotel_image']; ?>" alt="Hotel Image">
                        </div>
                        <div class="hotel-name">
                            ...
                            <a href="<?= home_url() ?>/hotel-details/?hotel-id=<?= $merged_result['hotel_id'] ?>" target="_blank">
                                <h3><?php echo $merged_result['hotel_name']; ?></h3>
                            </a>
                            <!-- View More Button -->

                        </div>
                        <div class="view-more">
                            <button type="button" onclick="location.href='<?= home_url() ?>/hotel-details/?hotel-id=<?= $merged_result['hotel_id'] ?>'">View Detail</button>
                        </div>
                    </div>
                <?php endforeach; ?>
                <!-- end update here -->
