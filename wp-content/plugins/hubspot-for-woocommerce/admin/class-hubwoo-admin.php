...
class Hubwoo_Admin {
	...
	public function hubwoo_redirect_from_hubspot() {
		...
		if ( isset( $_GET['code'] ) ) {
			...
			if ( isset( $_GET['mwb_source'] ) && 'hubspot' == $_GET['mwb_source'] ) {
	...
	public function hubwoo_review_notice() {
		...
		<div class="notice notice-success hubwoo-review-notice-wrapper">
			<div class="hubwoo-ocs-options hubwoo-pl-notice">
				<p>
					<strong><?php echo esc_html__( 'Liked our HubSpot WooCommerce Integration? Tell us what you loved in it.', 'hubspot-for-woocommerce' ); ?></strong>
				</p>
				<div class="hubwoo-wrapper-notice">
					<a target="_blank" href="https://woocommerce.com/products/hubspot-for-woocommerce/" class="hubwoo-btn--notific hubwoo-hide-rev-notice"><?php esc_html_e( 'Review Us.', 'hubspot-for-woocommerce' ); ?></a>
		...
		if ( 'yes' != get_option( 'hubwoo_connection_complete', 'no' ) ) {
			...
			<div class="notice notice-warning is-dismissible hubwoo-deactive-notice-wrapper">
				<div class="hubwoo-ocs-options hubwoo-pl-notice">
					<p>
						<strong><?php echo esc_html__(  'The HubSpot for WooCommerce plugin isn’t connected right now. To start sync store data to HubSpot ', 'hubspot-for-woocommerce' ); ?></strong>
						<a href="admin.php?page=hubwoo"><?php echo esc_html__(  'connect the plugin now.', 'hubspot-for-woocommerce' ); ?></a>
					</p>
				</div>
			</div>
		<?php
	...
	public static function hubwoo_ecomm_general_settings() {
		...
		$settings[] = array(
			'title'   => __( 'Enable Multi Currency	Sync', 'hubspot-for-woocommerce' ),
			'id'      => 'hubwoo_deal_multi_currency_enable',
			'desc'    => __( 'Enable to sync currency if you are using multi currency on your site.', 'hubspot-for-woocommerce' ),
			'type'    => 'checkbox',
			'class'   => 'hubwoo-ecomm-settings-checkbox hubwoo_real_time_changes',
		);
