...
class WPvivid_New_Staging_Push_Task_Ex
{
    public $task;
    public $options;
    public $current_job;
    ...
    public function setup_task($options)
    {
        ...
        $this->task['src_path']=untrailingslashit(ABSPATH);
        $this->task['des_path']= $options['site_data']['path'];

        $this->task['old_prefix']=$wpdb->base_prefix;
        $this->task['new_prefix']=$options['site_data']['prefix'];
        ...
        $this->task['new_site_url']=$options['site_data']['site_url'];
        $this->task['new_home_url']=$options['site_data']['home_url'];
        ...
        else if($this->task['is_mu_single'])
        {
            $this->setup_mu($options);
        }
        ...
        $this->setup_copying($options['staging_options']);
    }
    ...
    public function setup_mu($options)
    {
        ...
        if($options['is_mu'])
        {
            ...
            if(isset($options['all_site'])&&$options['all_site']==1)
            {
                ...
                foreach ($subsites as $subsite)
                {
                    ...
                    if(array_key_exists($subsite_id,$mu_site_list))
                    {
                        if($mu_site_list[$subsite_id]['tables']==0)
                        {
                            ...
                            $mu_exclude_table=$this->get_table_list($wpdb,$prefix,$mu_exclude_table);
                        }
                        else
                        {
                            $this->task['database_check']='1';
            ...
            if(isset($options['mu_main_site'])&&$options['mu_main_site']['check'])
            {
                ...
                {
                    $prefix=$wpdb->get_blog_prefix($options['mu_main_site']);
                    $mu_exclude_table=$this->get_table_list($wpdb,$prefix,$mu_exclude_table);
                }
            ...
            else
            {
                $uploads_path = $this->task['src_path'] . '/wp-content/uploads';
                $this->task['mu_upload_include'][] = '#^' . preg_quote($this->transfer_path($uploads_path . DIRECTORY_SEPARATOR . 'sites'), '/') . '#';
                $prefix=$wpdb->get_blog_prefix($options['main_site_id']);
                $mu_exclude_table=$this->get_table_list($wpdb,$prefix,$mu_exclude_table);
            }
        ...
        else if($options['is_mu_single'])
        {
            ...
            if($options['mu_single_site_id']==1)
            {
                $this->task['new_prefix']=$options['site_data']['prefix'];
            }
            else
            {
                $this->task['new_prefix']=$options['site_data']['prefix'].$options['mu_single_site_id'] . '_';
            }

            $this->task['new_site_url']=$this->get_mu_site_url($this->task['new_prefix']);
            $this->task['new_home_url']=$this->get_mu_home_url($this->task['new_prefix']);
    ...
    public function get_mu_site_url($site_prefix)
    {
        $db=$this->get_des_db_instance();
        $site_url=$this->task['new_site_url'];
        $options_name=$site_prefix.'options';
        $site_url_sql = $db->get_results( $db->prepare( "SELECT * FROM $options_name WHERE option_name = %s", 'siteurl' ) );
    ...
    public function get_mu_home_url($site_prefix)
    {
        $db=$this->get_des_db_instance();
        $home_url=$this->task['new_home_url'];
        $options_name=$site_prefix.'options';
        $home_url_sql = $db->get_results( $db->prepare( "SELECT * FROM $options_name WHERE option_name = %s", 'home' ) );
    ...
    public function setup_copying($staging_options)
    {
        $index=0;
        $this->task['jobs']=array();
    ...
    public function do_finish_staging()
    {
        if( $this->task['need_rename_tables'])
        {
            ...
            foreach ($new_tables as $table)
            {
                ...
                $db->query("RENAME TABLE {$table} TO {$new_table}");
            }
        ...
        $this->log->WriteLog('Updating the staging site completed successfully.','notice');
        $this->task['status']['resume_count']=0;
        $this->update_task();
    ...
    public function reset_staging_data()
    {
        ...
        $db->delete( "{$prefix}options", array( 'option_name' => 'wpvivid_staging_finish' ) );
        ...
        {
            $db->delete( "{$prefix}options", array( 'option_name' => 'wpvivid_staging_init' ) );
        ...
        $update_query =$db->prepare("UPDATE {$prefix}options SET option_value = %s WHERE option_name = 'blog_public'", '1');
        if ($db->get_results($update_query)===false)
        {
            $error=$db->last_error;
            $this->log->WriteLog($error, 'Warning');
        ...
        $db->delete( "{$prefix}options", array( 'option_name' => 'wpvivid_staging_data' ) );
