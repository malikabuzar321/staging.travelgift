...
class WPvivid_Staging_crypt
{
    ...
    public function __construct($public_key)
    {
        ...
        if(!class_exists('Crypt_Rijndael'))
            include_once WPVIVID_STAGING_PLUGIN_DIR . 'Crypt/Rijndael.php';

        if(!class_exists('Crypt_RSA'))
            include_once WPVIVID_STAGING_PLUGIN_DIR . 'Crypt/RSA.php';

        if(!class_exists('Math_BigInteger'))
            include_once WPVIVID_STAGING_PLUGIN_DIR . 'Math/BigInteger.php';
