...
class WPvivid_Staging_Copy_DB_Ex
{
    ...
    public function set_staging_site_data()
    {
        ...
        if($this->task->is_restore())
        {
            ...
            $need_retain_options = $this->task->get_need_retain_options();


            foreach($need_retain_options as $key => $option)
            {
                $update_query = $db->prepare("INSERT INTO {$prefix}options VALUES ('', %s, %s, '') ON DUPLICATE KEY UPDATE option_value=%s", $key, $option, $option);
                $wpvivid_staging->log->WriteLog($update_query, 'notice');
                if ($db->get_results($update_query)===false)
                {
                    $error=$db->last_error;
            ...
            $insert_query =$db->prepare("INSERT INTO {$prefix}options (option_name,option_value) VALUES ('wpvivid_staging_need_reset_schedules',%d)", 1);
            $wpvivid_staging->log->WriteLog($insert_query, 'notice');
        ...
        else
        {
            ...
            {
                $src_db=$this->get_db_instance(false);
                $wpvivid_pro_user_res = $src_db->get_results( $src_db->prepare( "SELECT option_value FROM {$this->old_prefix}options WHERE option_name = %s", 'wpvivid_pro_user' ) );
                ...
                $wpvivid_dashboard_info_res = $src_db->get_results( $src_db->prepare( "SELECT option_value FROM {$this->old_prefix}options WHERE option_name = %s", 'wpvivid_dashboard_info' ) );
                ...
                $wpvivid_staging->log->WriteLog($delete_query, 'notice');
                ...
                $wpvivid_common_settings_res = $src_db->get_results( $src_db->prepare( "SELECT option_value FROM {$this->old_prefix}options WHERE option_name = %s", 'wpvivid_common_setting' ) );
                ...
                $wpvivid_schedule_settings_res = $src_db->get_results( $src_db->prepare( "SELECT option_value FROM {$this->old_prefix}options WHERE option_name = %s", 'wpvivid_schedule_addon_setting' ) );
                ...
                $wpvivid_incremental_schedule_res = $src_db->get_results( $src_db->prepare( "SELECT option_value FROM {$this->old_prefix}options WHERE option_name = %s", 'wpvivid_incremental_schedules' ) );
                ...
                $wpvivid_upload_setting_res = $src_db->get_results( $src_db->prepare( "SELECT option_value FROM {$this->old_prefix}options WHERE option_name = %s", 'wpvivid_upload_setting' ) );
    ...
    public function create_table($table_name)
    {
        ...
        if( isset($result[0]['Create Table']))
        {
            ...
            {
                $new_db->query('SET SQL_REQUIRE_PRIMARY_KEY=0;');
            }
    ...
    public function replace_table(&$table,$count)
    {
        ...
        if(is_multisite())
        {
            ...
            if(substr($table['name'], strlen($this->replace_prefix))=='site')
            {
                if($this->task->is_subdomain_staging())
                {
                    $wpvivid_staging->log->WriteLog('Replace table subdomain staging: true', 'Warning');
                    $wpvivid_staging->log->WriteLog('update mu site', 'notice');
                ...
                {
                    $wpvivid_staging->log->WriteLog('Replace table subdomain staging: false', 'Warning');
                }
            }
        ...
        if($sum>0)
        {
            ...
            $wpvivid_staging->log->WriteLog('Start replacing '.$table['name'].' prefix from '.$start_row. ' row.', 'notice');
    ...
    private function rename_table(&$table)
    {
        ...
        if ( ! $db->get_var( $query ) )
        {
            $wpvivid_staging->log->WriteLog('Rename table not existed. Skip table name: '.$table_name, 'notice');
        }
        else
        {
            if(substr($table_name, strlen($this->replace_prefix))=='options')
            {
                ...
                global $wpvivid_staging;
                $tasks = $wpvivid_staging->option->get_option('wpvivid_staging_task_list');
            }
            ...
            if(substr($table_name, strlen($this->replace_prefix))=='options')
            {
                ...
                $wpvivid_staging->option->update_option('wpvivid_staging_task_list',$tasks);
            }
        }
